...
This document is the canonical roadmap once `05-real-node-design.md` is shipped. Read after `06-node-registry-pr.md` and `07-tech-stack-rationale.md`. **Current end-to-end architecture** (graph engine, DB, spam pipeline, auth): [`docs/ARCHITECTURE.md`](ARCHITECTURE.md). **Server-side graph execution plan:** [`docs/13-server-graph-executor-roadmap.md`](13-server-graph-executor-roadmap.md). Spam triage vertical: [`docs/12-spam-detection-app.md`](12-spam-detection-app.md). **Index:** [`docs/README.md`](README.md).

The “What we already have” section below is a **historical anchor** for early Stage A planning; much of it is now superseded by shipped persistence, Postgres, and spam features described in ARCHITECTURE.md.
...
| **D4** | **Eval harness.** A `data/evals/` folder with hand-written `(query, goldChunkKeys)` pairs + `npm run eval` (BM25 default; `--cosine` uses OpenAI embeddings) reporting recall@K and MRR. **Repo:** `data/evals/default.json`, `src/eval/`, `scripts/run-eval.ts`. **This is the single thing that separates a serious RAG project from a weekend project.** | 1 day |
