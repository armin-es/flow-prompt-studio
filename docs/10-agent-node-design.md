...
> **Status:** Implemented (`AppAgent`, `AppTool`, **`AppToolsJoin`** for TOOLS merge, **`POST /api/complete/tools`**). This doc remains the design reference. Read after `09-agents-strategy.md` (which gates this work behind RAG being usable — see Gate G1 there).
>
...
TEXT fan-in stays on **`AppJoin`**. **`AppToolsJoin`** is the small companion node that merges two **`TOOLS`** payloads (same drag-time type check).
