...
The Stage-B pipeline **is a graph in the studio** — same engine, same nodes (mostly new), same audit. That graph is selectable per ingest source ("blogs" vs. "comments" vs. "DMs" can run different graphs).
...
| Route | Purpose |
| --- | --- |
| `POST /api/spam/items` | Ingest a single item (text + author features + source). Returns `{id, status}`. |
| `POST /api/spam/items:bulk` | Ingest a batch (NDJSON or JSON array). |
| `POST /api/spam/webhook/:source` | Public webhook variant with a per-source HMAC. |
| `GET  /api/spam/items?status=&category=&q=&limit=&cursor=` | Reviewer queue. Cursor-paginated. |
| `GET  /api/spam/items/:id` | Detail: post + features + nearest known cases + policy excerpt + LLM rationale + previous decisions. |
| `POST /api/spam/items/:id/score` | Run the configured graph against this item (idempotent if status is `new`). |
| `POST /api/spam/items/:id/decision` | Reviewer action; appends to `spam_decisions`, sets `final_action`. |
| `POST /api/spam/items/:id/replay` | Re-run with current graph + corpora; doesn't change `final_action`, only logs a new `run`. |
| `GET  /api/spam/categories` / `POST` / `PATCH` | Manage categories and their backing corpora. |
| `GET  /api/spam/rules` / `POST` / `PATCH` | Manage hard rules. |
| `GET  /api/spam/metrics?from=&to=` | Aggregate precision/recall, queue size, override rate per category. |
