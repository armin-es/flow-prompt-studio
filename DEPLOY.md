...
- **`DB migration failed` / `connect ENETUNREACH` to an IPv6 address (`2600:…`) — Render often has no **IPv6 egress**; Supabase `db.*.supabase.co` may resolve to IPv6 first. **Fix:** The API sets **`dns.setDefaultResultOrder('ipv4first')`** in `server/db/client.ts` so **`pg`** resolves IPv4 first (no env var required). Redeploy after pulling that change. If it still fails, add **`NODE_OPTIONS`** = **`--dns-result-order=ipv4first`** on Render as a belt-and-suspenders check, or use Supabase’s **IPv4** options.
