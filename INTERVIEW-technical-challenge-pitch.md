...
- **Topology demos (TEXT):** `AppTee` (1→2 fan-out), `AppJoin` (2→1 concat), `AppPrefix`, `AppPick` (2→1 selective)—toolbar **Tee/Join** and **Pick 2→1** need no API; **Join+LLM** is two inputs → **Join** → **AppLlm** → **Output** (merge-then-prompt). **RAG** adds **`AppRetrieve`** (BM25/cosine) with preset **?demo=rag** (tee + retrieve + join + LLM). **Agent** (`?demo=agent`): **`AppTool`** leaves → **`AppToolsJoin`** → **`AppAgent`** (tool loop via **`POST /api/complete/tools`**).  
